...
def generate_response(llm,title,skills,experience,responsibilites,qualification,location,salary):
    template="""You are a HR of a well reputated company . You have to prepare job description for the requirements of job profile given below
    ...
    response = chain.invoke({"skills":skills,"title":title,"experience":experience,"responsibilites":responsibilites,"qualification":qualifications,"location":location,"salary":salary})
    st.write(response)
# Submit button
if st.button("Generate Job Description 🚀"):
    if job_title and skills and experience and responsibilities:
        generate_response(llm,job_title,skills,experience,responsibilities,qualifications,location,salary)  # You will implement this function
    else:
